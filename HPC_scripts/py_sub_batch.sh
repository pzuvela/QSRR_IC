...
sed -i "16s@.*@ncpus=$1@" py_sub.sh

# Replace the initial counter
sed -i "19s@.*@p=$2@" py_sub.sh

# Replace lines with submission script filenames
sed -i "26s@.*@rm -rf \${sub_dir}/\${p}_$4_$3_iters_$dt.sh 2> /dev/null@" py_sub.sh
sed -i "29s@.*@cat <<EOF >> \${sub_dir}/\${p}_$4_$3_iters_$dt.sh@" py_sub.sh

# Replace the lines with log & error file names
sed -i "35s@.*@#PBS -e \${err_dir}/\${p}_$4_$3_iters_$dt.err@" py_sub.sh
sed -i "36s@.*@#PBS -o \${logs_dir}/\${p}_$4_$3_iters_$dt.log@" py_sub.sh

# Replace the line with the python "call"
sed -i "52s@.*@python3.6 \${file} $3 \${p} \$ncpus $4 no \> \${output}/\${p}_$4_$3_iters_$dt.out@" py_sub.sh
# sed -i "52s@.*@python3.6 \${file} $3 \${p} \$ncpus $4 no" py_sub.sh

# Replace the line to submit the job sh script
sed -i "56s@.*@qsub \${sub_dir}/\${p}_$4_$3_iters_$dt.sh@" py_sub.sh
